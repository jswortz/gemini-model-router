...
class ClaudeCLIBackend(Backend):
    def __init__(self, cfg: BackendCfg):
        if cfg.binary is None:
            raise ValueError("claude_cli backend requires binary")
        self.cfg = cfg
        self.name = cfg.name
    ...
    async def invoke(
        ...
        *,
        ...
        # Claude Code JSON envelope: {"result": "...", "usage": {...}, "session_id": "..."}
        text = data.get("result") or data.get("text") or ""
        u = data.get("usage", {}) or {}
        in_tok = int(u.get("input_tokens", 0))
        out_tok = int(u.get("output_tokens", 0))
        # Cache tokens are kept in raw for later analysis but not added to billable in/out here.
        usage = Usage(
            input_tokens=in_tok,
